...
void PowerMonitor::update()
{
    ...
    if (m_adcHandle->State & HAL_ADC_STATE_REG_EOC)
    {
...
auto PowerMonitor::toTemperature(uint16_t rawValue) -> double
{
    ...
    static constexpr uintptr_t TsCal1Address = 0x1fff7a2c;
    static constexpr uintptr_t TsCal2Address = 0x1fff7a2e;

    static constexpr double TsCal1Temperature =  30.0;
    static constexpr double TsCal2Temperature = 110.0;

    static const uint16_t tsCal1Value = *reinterpret_cast<uint16_t*>(TsCal1Address);
    static const uint16_t tsCal2Value = *reinterpret_cast<uint16_t*>(TsCal2Address);

    static const double tsCalRatio =
            (TsCal2Temperature - TsCal1Temperature) / (tsCal2Value - tsCal1Value);

    return (rawValue - tsCal1Value) * tsCalRatio + TsCal1Temperature - JunctionToAmbient;
}
