...
void HostInterface::prepareBulkOutData(std::span<uint8_t> data)
{
    auto status = USBD_VENDOR_BulkPrepareRx(&m_deviceHandle, data.data(), data.size());
...
void HostInterface::sendBulkInData(std::span<const uint8_t> data)
{
    auto ptr = const_cast<uint8_t*>(data.data());

    auto status = USBD_VENDOR_BulkSendData(&m_deviceHandle, ptr, data.size());
    ASSERT(status == HAL_OK);
    UNUSED(status);
}
...
void HostInterface::sendBulkInDataWait(std::span<const uint8_t> data)
{
    auto ptr = const_cast<uint8_t*>(data.data());
...
void HostInterface::onInitComplete(void* pUserData)
{
    ...
    auto instance = reinterpret_cast<HostInterface*>(pUserData);
    instance->onInitComplete();
}

// ---------------------------------------------------------------------------------------------- //

inline void HostInterface::onInitComplete()
{
...
inline void HostInterface::onControlInReceived(uint8_t request, uint16_t length)
{
...
inline void HostInterface::onControlOutReceived(uint8_t request, uint16_t length)
{
    m_request = request;
    m_length = length;
...
inline void HostInterface::onControlRxReady()
{
...
inline void HostInterface::onBulkInComplete()
{
    if (m_bulkInWaiting)
...
inline void HostInterface::onBulkOutReceived(uint16_t length)
{
